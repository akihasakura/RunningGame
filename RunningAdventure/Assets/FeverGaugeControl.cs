...
public class FeverGaugeControl : MonoBehaviour {
	...
	void Update () {
		...
		if(localScore < FeverScore){
			...
			// FeverTime!
			// フィーバーゲージ表示
			gaugeObject.GetComponent<Renderer>().enabled = false;
			feverGaugeObject.GetComponent<Renderer>().enabled = true;
		}
	}
	...
	public void init(){
		...
		// フィーバーゲージ非表示
		gaugeObject.GetComponent<Renderer>().enabled = true;
		feverGaugeObject.GetComponent<Renderer>().enabled = false;
	}
}
