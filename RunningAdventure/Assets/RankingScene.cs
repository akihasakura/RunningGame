	...
	void UpdateRank()
	{
		...
		if( lBoard.topRankers != null && lBoard.neighbors != null && !isLeaderBoardFetched){	
			...
			*/
			...
			myName.GetComponent<GUIText>().text = _name;
			myRank.GetComponent<GUIText>().text = lBoard.currentRank.ToString();
			myScore.GetComponent<GUIText>().text = currentHighScore.score.ToString();
		}
